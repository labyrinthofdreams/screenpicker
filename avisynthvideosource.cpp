...
void AvisynthVideoSource::load(QString fileName)
{
    ...
    try
    {
        ...
        res = avsHandle.func.avs_invoke(avsHandle.env, "Import", arg, NULL);
        if(avs_is_error(res))
        {
        ...
        AVS_Value conv_tmp = avsHandle.func.avs_invoke(avsHandle.env, "ConvertToRGB24",
...
QImage AvisynthVideoSource::getFrame(unsigned frameNumber)
{
    ...
    const unsigned scanlineLength = info->width * 3;
    const int pitch = avs_get_pitch_p(frame, 0);
    const BYTE* data = avs_get_read_ptr_p(frame, 0);
